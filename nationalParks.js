"use strict";
// Function to enable dropdown behavior
        ...
        radio.addEventListener('change', () => {
            if (radio.checked) {
                ...
                if (radio.value === "option1") {
                    // If "option1" is selected, load states dropdown
                    initLoadDropdownStates(gifts1, true);
                    // Add event listener to states dropdown
                    dropdown.addEventListener('change', () => {
                        const selectedState = dropdown.value;
                        // Display park details for selected state
                        displayParkDetails(selectedState);
                    });
                } else if (radio.value === "option2") {
                    // If "option2" is selected, load park types dropdown
                    initLoadDropdownTypes(gifts2);
                    // Add event listener to park types dropdown
                    dropdown.addEventListener('change', () => {
                        const selectedType = dropdown.value;
                        // Display park details for selected type
                        displayParkDetailsByType(selectedType);
                    });
                }
            }
        });
...
// Function to load park types into dropdown
function initLoadDropdownTypes(data) {
    const locationList = document.getElementById("optionDropdown");
    locationList.innerHTML = ''; // Clear existing options
    data.forEach(item => {
        let theOption = document.createElement("option");
        theOption.value = item.type;
        theOption.textContent = item.type;
        locationList.appendChild(theOption);
    });
}

function initLoadDropdownStates(data, isState) {
    const locationList = document.getElementById("optionDropdown");
    locationList.innerHTML = ''; // Clear existing options
    data.forEach(item => {
        let theOption = document.createElement("option");
        if (isState) {
            theOption.value = item.state;
            theOption.textContent = item.state;
        } else {
            theOption.value = item.type;
            theOption.textContent = item.type;
        }
...
//states list -display//
function displayParkDetails(state) {
    const parkDetailsDiv = document.getElementById("parkDetails");
    parkDetailsDiv.innerHTML = ''; // Clear previous details

    const filteredParks = nationalParksArray.filter(park => park.State === state);
    if (filteredParks.length > 0) {
        ...
        filteredParks.forEach(park => {
            ...
            headers.forEach(headerText => {
                ...
                cell.textContent = park[headerText];
                row.appendChild(cell);
            });
            table.appendChild(row);
        });

        parkDetailsDiv.appendChild(table);
    }
}
...
// Function to display park details based on selected type
function displayParkDetailsByType(type) {
    const parkDetailsDiv = document.getElementById("parkDetails");
    parkDetailsDiv.innerHTML = ''; // Clear previous details

    // Filter parks based on whether location name includes the selected type
    const filteredParks = nationalParksArray.filter(park => park.LocationName.includes(type));
    if (filteredParks.length > 0) {
        const table = document.createElement("table");
        const headerRow = document.createElement("tr");
        const headers = ["LocationID", "LocationName", "Address", "City", "State", "ZipCode", "Phone", "Fax", "Latitude", "Longitude"];
        ...
        // Populate table with park details
        filteredParks.forEach(park => {
            const row = document.createElement("tr");
            headers.forEach(headerText => {
                const cell = document.createElement("td");
        ...
        // Append table to parkDetailsDiv
        parkDetailsDiv.appendChild(table);
    } else {
        // Display message when no parks are found for the selected type
        parkDetailsDiv.textContent = "No parks found for the selected type.";
    }
}

// Event listener for when DOM content is loaded
document.addEventListener('DOMContentLoaded', (event) => {
    enableDropdown(); // Enable dropdown behavior
});
